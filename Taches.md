...
- [ok] Système de design complet avec variables CSS
- [ok] **Palette couleurs** :
  - Client : bleu indigo `#4F46E5`
  - Opérateur : **bleu-vert paon** `#0D9B8C` (remplacement du violet)
- [ok] Police **Inter** (Google Fonts) sur tout le projet
- [ok] Composants : cards avec ombre, `balance-card` gradient, `action-btn` colorés
- [ok] `stat-card` avec icônes sémantiques (dépôt=vert, retrait=rouge, transfert=bleu)
- [ok] Tables épurées avec hover subtil
- [ok] Badges sémantiques pill (`badge-depot`, `badge-retrait`, `badge-transfert`)
- [ok] Fee table (tableau des barèmes) stylisé en teal
- [ok] Formulaires avec focus ring coloré et icônes préfixes
- [ok] Boutons avec gradient + ombre colorée + animation hover
- [ok] Pagination personnalisée
- [ok] Responsive mobile (breakpoints Bootstrap 5)
- [ok] Scrollbar personnalisée

### JavaScript (public/assets/js/app.js)
- [ok] Auto-dismiss des alertes flash après 5 secondes
- [ok] Toggle affichage mot de passe (page login opérateur)
- [ok] Confirmation avant soumission retrait / transfert

### Configuration & Déploiement
- [ok] `composer.json` — dépendances CI4 4.7.4 + SQLite
- [ok] `.env` — configuration environnement (base URL, CSRF, admin credentials)
- [ok] `spark` — CLI CodeIgniter 4 (format Bootstrap 4.5+)
- [ok] `public/index.php` — front controller CI4 4.5+ (`Boot::bootWeb`)
- [ok] `public/.htaccess` — réécriture URL Apache
- [ok] `README.md` — guide d'installation et d'utilisation
- [ok] Installation et correction des dépendances ThirdParty (Kint, Escaper, PSR/Log)
